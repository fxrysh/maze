...
;;
;; [0x0000, 0x0004]   u8[4]             W/A/S/D keys
...
;; [0x0c90, 0x0d90]   u8[32*32]         RLE compressed 2 BPP textures
;; [0x0d90, 0x0dac]   u8[28]            Color index (palette table @0x11b0)
...
(func (export "run")
  ...
  ;;
  ;; NOTE: $Dx / $Dy is used for the view direction, and $ray-x / $ray-y is used for
  ;; the movement vector.
  (global.set $ray-x
    (local.tee $Dx
    ...
    (block $winning
      (block $game
        (block $reset
          ...
          (if (i32.eqz (global.get $mode-timer))
            (then
              (global.set $mode (i32.const 4)) ;; Win.
        ...
        (loop $wall-loop
          ...
          (if
            (i32.ne
              ...
              ;; NOTE: $cell1 is the R/D cell
              (local.tee $cell1
                (i32.load8_u offset=0x0010
                  (i32.load8_u offset=0x00a1 (local.get $wall-addr)))))
            (then
              ;; Remove this wall by overwriting the last wall over it.
          ...
          (local.set $dot-product
            (f32.add
              (f32.mul
                ...
                ;; NOTE: Wall is stored as (x, y),(dx, dy),scale. Since we want
                ;; the normal, store (-dy / scale, dx / scale).
                (local.tee $normal-x
                  (f32.neg
                    (f32.div
                      (f32.convert_i32_s
      ...
      (if (i32.and
        ...
        (then
          ...
          (global.set $mode (i32.const 4)) ;; Win. Well, won.
          (global.set $mode-timer (i32.const 120)) ;; Reset position over time/
